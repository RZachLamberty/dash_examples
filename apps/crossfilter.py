...
layout = html.Div([
    html.H2("generic crossfilter example"),
    html.Div([
        html.Div(
            dcc.Graph(
                id='g1',
                # if selectedData is not specified then it is initialized
                # as None
    ...
    def callback(*selectedDatas):
        ...
        figure = {
            'data': [
                ...
                dict(
                    {
                        'x': dfrand[x], 'y': dfrand[y], 'text': dfrand.index,
                        'customdata': dfrand.index, 'mode': 'markers',
                        'opacity': 0.1
                    },
                    **trace_template
                ),
